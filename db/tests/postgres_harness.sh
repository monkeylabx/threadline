...
postgres_test_start() {
  ...
  case "$POSTGRES_TEST_SUITE" in
    atlas_migration | organization | member | space | channel_dm | channel_membership | resource_acl | authorization_current | transactional_outbox | transactional_outbox_worker_ops | migration) ;;
