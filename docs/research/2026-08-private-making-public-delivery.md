...
这不把 Threadline 降格为现有 IM 旁的“交付插件”。Codex、Claude 这类个人 Agent 客户端缺少人际频道、成员关系、日常沟通和团队决定的主界面；Threadline 的产品类别仍是完整的**人对人 IM**。私人制作与显式交付不是侧车功能，而是团队选择这种 IM 的理由。Slack 是这个定位最直接的压力测试，见[Slack 对 Threadline 定位的压力测试](./2026-08-slack-threadline-positioning.md)。
...
不是泛泛地说“我们也有 AI，所以替换飞书/钉钉/Slack”，而是让一个完整团队选择 Threadline 的人际频道作为它日常协作的一部分，并获得：

> 让一个真实团队把它现有的“私下用 AI 做出来，再截图/贴链接/整理几句发群里”的交付环，换成“私下制作 → 发布 Artifact → 频道审阅/决定 → 退回修订 → 新版交付”。

试点可以与既有 IM 并行，避免一开始迁移全公司的聊天历史；但并行只是验证/迁移策略，不是产品定位。Threadline 的价值只有在需求、制作和交付都留在同一个人际 IM 中才完整成立。若这个环在一个团队中形成日常习惯，才有依据扩展为该团队的主要 IM，而不是永远停留在侧车工具。
