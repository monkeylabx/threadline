...
XCTest 随 Xcode 分发，本机不可用，因此新增了一个不依赖 XCTest 的可执行
harness：[`apps/ios/Sources/ThreadlineFFIHarness`](../../apps/ios/Sources/ThreadlineFFIHarness/main.swift)，
复用同一套 `ThreadlineIOSHost` Facade 源码。

```bash
cargo build --package threadline-client-ffi --locked
THREADLINE_FFI_LIBRARY_DIR="$PWD/target/debug" \
  swift run --package-path apps/ios ThreadlineFFIHarness
```
...
| 缺口 | 原因 | 影响 |
| --- | --- | --- |
| iOS Simulator 实跑 | 需完整 Xcode（含 XCTest、iphonesimulator SDK） | `scripts/ci/run-ios-simulator-ffi.sh` 未执行 |
| Android Emulator 实跑 | 需 JDK 17 + Android SDK/NDK/Emulator | `.so` 未构建，Kotlin↔Rust 未实跑 |
| Android JVM 单元测试 | 需 JDK + Gradle + AGP | `testDebugUnitTest` 未执行 |
| **iOS / Android 真机** | 无设备、无签名证书 | **P00-07 与 ADR-0001 明文要求的就是真机** |
| CI 实际运行结果 | 本机无 `gh`，分支未合并、未见 PR | 无法确认 `build.yml` 曾成功跑过 |
...
### 4.2 Simulator/Emulator 脚本仅在 CI 镜像上可用

`scripts/ci/run-ios-simulator-ffi.sh` 与 `run-android-emulator-ffi.sh` 依赖固定的
Xcode 26.6 / Android 镜像。开发机若缺 Xcode，此前没有任何可跑的 Swift 侧验证——
本次新增的 `ThreadlineFFIHarness` 填补了这一段，可作为开发机与 PR 的快速前置检查。
...
**门禁层面：P00-07 尚不可关闭。** 缺的是执行证据而非设计证据：

1. 在装有 Xcode 26.6 / Android SDK 的机器或 CI 上跑通 `build.yml` 的 `apple` 与
   `android` job，留存 `.xcresult` 与 Instrumentation 报告。
2. 补 T010-B 的 iOS/Android **真机** 验证（加载、签名、后台/进程回收、内存压力）。
3. 将 `t009 → t010 → t010a` 这条未合并链合入 `main`——当前 `main` 上 `client-ffi`
   仍是空壳，任何下游工作都拿不到这些成果。

建议 M0 对 P00-07 记为**条件通过（证据待补）**，并把上述 1–3 项作为门后立即项；
若必须严格按"真机通过"判定，则 P00-07 应判为未过，但风险等级可从"不可逆风险"
...
# Swift 边界（无需 Xcode）
cargo build --package threadline-client-ffi --locked
THREADLINE_FFI_LIBRARY_DIR="$PWD/target/debug" swift run --package-path apps/ios ThreadlineFFIHarness
