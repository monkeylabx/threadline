...
状态：M0 工程范围基线 + V33 已确认产品交互（静态原型，非完整功能实现）
更新：2026-09-06；原始基线日期：2026-07-24
...
### 产品价值：让个人 AI 工作成为团队可以接手的成果

Threadline 的本质是 **IM + Agent Runtime**：人和人的日常沟通是主干，个人与 AI 的工作也是一等使用场景。
不是把群聊改造成多人围着 Agent 聊天，也不以 Agent 数量作为产品价值。

- **沟通和制作在同一个产品里衔接。** 频道与私聊承载团队沟通；私人工作承载个人与 AI 的持续对话、
  显式选择的上下文和本地工作目录。工作时仍能找到频道与消息提醒，不必把两个模式完全隔开。
- **公开的是选定成果，不是全部私人过程。** 从频道引用内容发起私人工作，不代表频道获得本地文件、
  Prompt 或运行过程的访问权。用户明确发布后，团队才能看到所共享的成果及其授权范围内的信息。
- **成果可以被团队直接接手。** 频道中的交付不止一段机器人文字：成果可以预览、讨论、确认并继续流转，
  关联来源、版本与责任人，减少复制粘贴、重复解释背景和追问“哪个是最终版”。
- **IM 不依赖 AI 才能成立。** 即使模型或本地 Runtime 离线，成员仍能沟通与协作。

商业价值假设是：降低个人 AI 产出进入团队协作时的交接和确认成本，而非出售“更多 Agent”或查看员工私人过程。
当前尚未验证客户付费意愿，也不把隐私边界包装成付费开关；应以真实团队的交接耗时、背景重复说明次数、
成果被同事查看与继续使用的比例，以及持续使用情况来验证。已有 IM 的 AI 能力需要正面比较，不能预设它们只有文字机器人。

本次 HTML 原型验证交互与信息层级，尚不代表真实消息同步、权限系统、Runtime 执行或成果存储已经实现。
...
6. **授权范围内的执行过程可观察。** 团队任务按频道与任务权限展示状态、工具活动、审批、产物、成本和责任人；
   私人工作不会因引用频道内容而自动公开，团队不能据此查看成员本地运行过程。
...
| --- | --- | --- | --- |
| 人际 Channel 不成为 Agent Chat | 团队继续正常聊天，不被制作过程淹没 | Human 消息、显式触发的 Agent 提议及获权团队任务状态、已发布交付 | 私人 Prompt/回复流不得自动进入频道 |
...
[打开已确认的 V33](./prototype/index.html?screen=channel&prototype=im-agent-fusion&variant=A&ui=v33&viewport=desktop)。
消息、工作、处理在同一应用导航中保持可达；第二栏显示当前列表，主区用于人际消息或个人 AI 工作。
第一栏默认104px，可拖动88–160px、折叠为48px；第二栏默认180px，可拖动112–360px，始终保留当前列表。
外层图标右上橙点表示有更新；第二栏深橙色数字在行尾，无底色。企业与个人入口左对齐；输入框固定在工作区底部。

私人工作与共享 Task 的区别见 [ADR-0005](./adr/0005-private-work-publication-boundary.md)。UI 选择已确认，
存储/发布/受众隔离契约仍待评审。此前共享任务验收没有因此覆盖私人工作，不能据本原型宣布隐私功能已实现。

#### 历史探索记录（不作为现行规范）

下列 V3–V33 按时间保留决策过程；冲突时以“当前评审基线”和最后确认版本为准，不叠加各版布局。
...
其中 `A` 的 Agent 表面直接占据应用工作区，不再嵌套独立卡片或窗口；它保留个人 Agent 会话历史和经用户
授权选择的本地工作目录。IM 只以未读提示与临时通信浮层进入，关闭后恢复到同一会话、目录和滚动位置。

V6 将 `A` 收敛为统一对话模型：Channel、DM 和私人 Agent 工作共享同一套可收起导航，私人工作可以作为
某个 Channel 的可见关联分支，也可以保持“未关联频道”。这个 UI 关联不改变领域边界——Channel 仍不是
Agent Session；Runtime 只能读取用户显式引用的消息，私人分支也不会自动成为团队消息。

V7 移除 Agent 工作中的独立消息弹层：提及和未读直接显示在统一导航的 Channel 上。Agent 消息历史独立滚动，
输入框悬浮在工作区底部且不以分隔线切断对话，并沿用 IM 聊天框的结构；“新建工作”默认创建未关联频道、
仅自己可见的 Agent Session，后续再由用户显式引用频道消息或共享成果。

V9 将创建动作放回各自的信息层级：Channel、私人 Agent 工作和 DM 的栏目标题分别提供新建入口，统一搜索覆盖
频道、工作和组织成员。应用功能轨与对话导航保持两级结构，但前者缩窄且可完全收起，后者也降低宽度以把注意力
留给当前对话或 Agent 工作。

V10 取消独立应用功能轨，把侧栏收敛为两个稳定栏目：`沟通` 混合最近 Channel 与 DM，`私人工作` 承载本地
Agent Session。`需要处理` 只是有待审批或 Agent 结果时出现的紧凑入口，不汇总 `@提及`；提及数量直接留在
对应 Channel 上。整条侧栏可一次性收起，让沟通可达但不持续挤占私人工作空间。

V11 保留 `沟通` 与 `私人工作` 两个栏目同时可见，不把它们改成互斥 Tab。搜索下方提供“发消息 / 新工作 /
待处理”三个独立快捷按钮：按钮只触发动作或临时浮层，不替换侧栏内容。侧栏收起后保留企业 Logo、快捷按钮
状态和 Runtime 在线点，折叠控件仅显示无边框箭头。

V12 撤销顶部快捷按钮，恢复单个“需要处理”入口；发起沟通和新建私人工作的动作回到各自栏目。侧栏控制不再
占用右侧工作标题，而是成为左侧导航自身的图标：展开时位于搜索旁，收起后位于企业 Logo 下方。

V13 将侧栏控制收敛为无外框的“竖线 + 方向箭头”，避免面板图标的笨重感。收起状态继续显示需要处理、沟通、
私人工作三个分类图标与实时角标；点击任一分类会展开侧栏并定位到对应栏目。

V14 采用三层导航加内容区：企业栏、应用栏、随应用变化的上下文侧栏，以及聊天或私人 AI 工作区。全局搜索
进入桌面应用顶部，覆盖消息、频道、工作和成员；应用栏上的未读和待处理角标在用户专注 AI 工作时仍然可见。

V15 为企业栏与第三层上下文侧栏提供独立折叠。企业栏收起后，当前企业 Logo 移到应用栏顶部作为恢复入口；
上下文侧栏收起后，企业栏和应用栏继续保留，内容区扩展到释放出的空间。全局搜索只在内容区上方居中，不跨越
应用栏和上下文侧栏的分隔线；列表收起时，搜索随内容区向左扩展。

V16 参考 Slack 的分层关系重置折叠入口，而不是复制其视觉主题：企业切换器从企业栏顶部收起，收起后由应用栏
顶部的当前企业 Logo 恢复；频道或工作列表从其与内容区之间的边缘收起，收起后在应用栏中提供“列表”恢复入口。
全局顶部栏只保留历史导航、搜索与 Runtime 状态，不再混入侧栏管理。

V17 取消常驻企业栏，将企业 Logo、名称、切换菜单和折叠控制合并到全局顶部栏。默认布局缩减为应用栏、上下文
侧栏和内容区三栏；收起企业信息时不改变工作区宽度，只隐藏企业名称与控制并保留 Logo，Logo 成为恢复入口。
企业切换是低频全局操作，不再与消息、工作和待处理等高频应用入口争夺纵向空间。

V18 将企业 Logo 与名称合为同一个企业菜单入口，取消独立企业收起、无行为的前进后退和列表边缘箭头。
顶部只保留一个位置稳定的列表折叠按钮；收起列表时公司名称随之隐藏，Logo 仍打开同一个企业菜单。
个人头像固定在应用栏左下角，消息、工作、处理及列表折叠状态均保留它。个人菜单展示身份并可预览在线/勿扰；
菜单支持再次点击、点击外部和 Escape 关闭。企业菜单仅展示当前企业，跨企业切换仍待实现。

V19 修正上述顶栏归属：企业 Logo 回到应用栏顶部、窗口控制区下方；个人头像继续固定在底部。
全局顶栏预留原生窗口控制空间，提供单一列表折叠、后退、前进、最近访问和搜索入口；不增加无用途的更多菜单。
折叠只隐藏上下文列表，企业及应用入口、角标、个人头像保持可见。历史原型仅记录本次预览的栏目访问，
不持久化或向团队共享私人记录；无可访问历史时禁用对应方向。网页红黄绿仅为位置示意，原生窗口集成仍待验证。
依据和平台差异见 [Slack 桌面导航调研](research/slack-desktop-navigation.md)。

V20 更正折叠契约：顶栏折叠按钮只将第一栏切为纯图标，隐藏消息／工作／处理的文字并略微收窄；
企业 Logo、个人头像、应用选中态和角标保留。第二栏不折叠、不隐藏，频道／私人工作／需要处理列表
继续显示栏目及当前条目。悬停图标可见名称，读屏名称独立于隐藏文字；切换紧凑状态不改变当前栏目或草稿。

V21 明确展开态为 144px 的横排图标＋文字导航，企业 Logo 旁显示企业名称；收起态为 48px 纯图标栏。
两态相差 96px，空间归还主内容区；第二栏固定 228px，不通过隐藏当前列表换取空间。

V22 将第二栏改为可拖拽调整：默认 228px，下限 180px，上限 360px，并根据窗口宽度为主内容保留至少
320px（桌面可容纳范围内）。右边界提供 8px 命中区及悬停反馈，双击恢复默认；聚焦后左右键微调、
Home/End 到边界，Escape 取消当前拖拽。第一栏折叠及栏目切换保留所选宽度；窗口不足时临时收窄，
空间恢复后恢复偏好值。仅本次预览内存保存，不持久化、不影响移动端渲染器。

V23 第一栏默认收窄至 104px，企业只显示 Logo，全称在悬停和菜单内；消息／工作／处理保留横排文字。
第一栏展开时也可独立拖拽（88–160px）、键盘微调及双击复位；折叠为 48px 时隐藏该拖拽柄，
重新展开恢复用户宽度。两栏分别保留内存偏好，在可容纳范围内互不重置。

V24 企业 Logo 与底部个人头像沿第一栏中轴水平居中，随拖拽和折叠自适应；应用图标＋文字仍保持原有排列。

V25 按通知标记调研分层：消息导航仅显示小点，普通频道未读仅加粗；提及数量以统一的 18px 标记
放在频道行末，不再显示微型 `@1`。处理入口在文字右侧显示中性色数量，折叠后以小点代替，详情仍在
第二栏。提示和无障碍名称保留具体语义，不新增提及入口。此轮只验证标记外观，未接入真实已读清除逻辑。

V25 同时统一企业／个人入口：38px 点击区、32px 图形、9px 圆角、相同底色和描边，沿第一栏中轴居中。
个人在线点独立定位于角落，不占图形布局；企业与个人身份通过字样、悬停提示和各自菜单区分。

V26 覆盖 V25 的混合提醒样式：两栏共用 `nav-signal-template` 和 `.nav-signal`，统一为行尾18px槽位内的
6px圆点。不再分别显示图标顶部状态、数字徽标或 `@1`；折叠态圆点仍垂直居中在行尾。
数量和含义放在所属行的 title/无障碍名称里；普通未读仍可加粗。成员在线状态不占用消息行尾提醒位。
企业／个人入口保留统一图形样式，但按用户要求改为第一栏左对齐，不再居中。本轮仍为静态状态原型。

V27 将共用提示组件改为中性灰数字（18px、同色同圆角），不再使用易与在线状态混淆的绿色点。
两栏及折叠态使用同一组件；0/无数量不渲染，超过9显示9+，完整数量由所属行提示说明。
折叠态图标与数字并排而非叠加；企业和个人入口继续左对齐。示例状态不代表真实通知计数已接入。

V28 按用户明确的层级规则拆分共用组件的显示模式：外层 `status` 只显示图标圆角右上方的灰色状态标记，
三项一致且不显示数字；第二栏默认 `count` 在行尾显示数量。展开／折叠不改变这套规则；0不显示，
完整语义保留在所属入口提示中。身份入口继续左对齐，不混用提醒状态和成员在线状态。

V29 第二栏可拖动至112px（原180px），默认228px、上限360px及双击恢复不变。
按栏宽自适应：低于180px缩小横向留白，处理类型标签移至标题下方；低于140px隐藏辅助说明。
名称省略但保留悬停全文及无障碍名称，数量、新建按钮、当前栏目和选中项始终可见；拉宽恢复完整信息。
顶部窗口操作保留独立最小空间，不因第二栏收窄与搜索重叠。

V30 外层通知状态使用橙色小点，仍锚定图标右上圆角；第二栏数量去掉灰色底，保留纯数字和原有对齐槽位。
层级、计数语义及窄栏自适应不变；橙色仅表示有新状态，不表示在线或执行故障。

V31 第二栏通知数字使用深橙色 `#a95113`，无底色，位置与字号不变；与外层橙色状态点形成同一提醒色系。

V32 按用户反馈将会话数量移到会话图标右上方，不再放在行尾。频道使用统一圆角#图标，私聊保留成员头像；
两者共用图标容器。数字继续深橙色、无底色，名称紧跟图标，窄栏仍保留文字省略。未读数为0的会话不显示数字。

V33 数字恢复至第二栏行尾最右侧，深橙色、无底色；保留频道图标和私聊头像，取消图标角标。
第二栏默认及双击恢复宽度改为180px（原228px），112px下限与360px上限不变，窄栏自适应保留。
...
- **消息：** 第二栏展示频道与私聊，提及/未读留在所属会话；不增加独立“@提及”一级入口。
- **工作：** 第二栏展示个人 AI 工作历史和新建入口；主区用于持续对话、选定上下文/目录及成果。
- **处理：** 需本人作出决定的审批、结果确认等；不复制一份普通消息列表。
- **顶部：** 窗口控制预留区、导航折叠、后退/前进/历史和统一搜索；原生宿主接入另行验证。
- **企业与个人：** 第一栏上/下方保留入口，折叠后仍可见；不挤占窗口关闭区域。

文件与产物、Agent 目录、Runtime 和管理等领域功能仍在产品范围及原型路由中；功能清单不等于全部常驻一级导航。
...
以下为领域页面清单及历史截图，不是 V33 导航的截图证明。当前布局以可交互入口为准；历史截图不重新生成，
Mobile 沿用内部渲染器，V33 桌面评审不代表移动端视觉、真机或完整无障碍验收通过。
...
| Inbox | 历史聚合路由保留；V33 日常提及/未读留在会话，需决定的事项进入处理 |
| Search | 消息、文件、任务和成员统一检索，返回结果前重新校验权限 |
| Files | 频道附件、工作区文档和 Agent Artifact 使用同一权限表达 |
| Task Run | 共享 Task 在获权范围内展示执行活动；私人工作不向频道自动公开过程 |
...
获权团队成员观察共享 Task 的结构化活动，不需要阅读 Runtime 原始日志；此页面不能用来展示其他成员的私人工作过程。
