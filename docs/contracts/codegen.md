...
The ellipses are schematic, not accepted values. Provenance is one of four exact forms. `official-binary` and `official-package` reference all applicable source names. `source-built` records one source archive, every authenticated bundle or host builder, the exact build command, and only the evidence claim `single-build-output-verified`; it does **not** call the resulting binary an upstream release or claim reproducible builds. The verifier proves that the reviewed bundle contains exact bundle inputs and output, and directly authenticates any admitted host Xcode builder before generation. It still does not independently observe the historical build command; the release record must retain the clean-runner build log separately. `protocol-stub` references protocol fixtures and is rejected by both formal modes. An invalid, unverifiable, or incompletely sourced builder cannot be promoted by adding a self-authored attestation.
...
from `toolchains.json`. The v5 formal release profile is currently restricted
to the protected `macos-26` Integration runner on `darwin-arm64`, because Swift
source builds require the pinned authenticated Xcode host builder; Linux or Windows
...
`proto/tools/create-codegen-bundle.mjs` is the deterministic packaging boundary used by the protected prepare job. It consumes an Integration-owned schema-1 build spec that names already-downloaded source artifacts, authenticated builders, isolated runtime-closure roots, exact tool paths, and provenance. It copies those inputs outside the repository, enumerates every file and relative symlink, computes closure and executable digests, and emits schema-5 `manifest.json` plus `manifest.sha256`. It does not download, build, authenticate, or approve tools; the separate review step must inspect that immutable artifact and supply its exact manifest digest to the verify job.
